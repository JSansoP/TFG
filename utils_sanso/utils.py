...
def normalize_audio(filepath):
    ...
    subprocess.run(["ffmpeg", "-i", filepath, "-ac", "1", "-ar", "22050", "-acodec", "pcm_s16le", filepath.replace(".wav","tmp1.wav"), "-y", "-loglevel", "error", "-hide_banner"])

    os.remove(filepath)

    #Now we normalize the audio file
    #command: ffmpeg -i in.wav -filter:a "speechnorm=e=6" out.wav
    subprocess.run(["ffmpeg", "-i", filepath.replace(".wav","tmp1.wav"), "-filter:a", 'speechnorm=e=6', filepath.replace(".wav","tmp2.wav"), "-y", "-loglevel", "error", "-hide_banner"])
