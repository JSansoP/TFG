...
def main(args):
    ...
    cut_audio_and_generate_metadata(out_folder, transformed_audio_path)



def cut_audio_and_generate_metadata(out_folder: str, transformed_audio_path: str) -> None:
    df = pd.read_csv(os.path.join(out_folder, "transcription.tsv"), sep="\t", header=0)
    with open(os.path.join(out_folder, "metadata.txt"), "r") as f:
        for index, row in df.iterrows():
            ...
            f.write('/content/tacotron2/wavs/{0}.wav|{2}\n'.format(str(index+1), cleaned_text))
